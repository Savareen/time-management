...
public class ApplicationSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests()
				.antMatchers("/", "/images/*", "/guest/*", "/index", "/css/*", "/js/*", "/login", "/register",
						"/registration")
				.permitAll()
				.antMatchers("/admin/**").hasRole("ADMIN").antMatchers("/user/*").hasRole("USER").anyRequest()
				.authenticated().and().formLogin()
				.loginPage("/guest/login").successHandler(applicationSuccessHandler()).failureUrl("/login-error")
				.and().rememberMe()
	...
	@Bean
	public AuthenticationSuccessHandler applicationSuccessHandler() {
